...
def determine_order():
    for _ in range(9):
      ...
      #  print(farthest())
       ...
       # print(nearest(e_drone.delivery_location[e_drone.order[-1]]))
...
def generate_csv():
    with open('sequenced_manifest_original.csv','w') as f:
        ...
        while(i < len(e_drone.order)):
            ...
            thewriter.writerow(['DELIVERY',box_id,dest])
            i += 1
            box_id = e_drone.order[i]
            dest = e_drone.return_location[box_id]
            thewriter.writerow(['RETURN',dest,box_id])
            i+=1
        #print("here")
